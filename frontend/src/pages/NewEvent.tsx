...
export default function NewEvent() {
  ...
  const [end, setEnd] = React.useState<string>(() => new Date(Date.now() + 3*24*3600*1000).toISOString().slice(0,10))
  ...
  const create = useMutation({
    mutationFn: () => {
      ...
      return api.createEvent({
        ...
        price_items,
        holiday_country_code: holidayCountry.trim() ? holidayCountry.trim().toUpperCase() : undefined,
  ...
  const disabled = !name.trim() || !start || !end || !currency.trim() || items.every((it) => !it.name || parseMoneyToMinor(it.price || '') <= 0)
        ...
        <div className="vstack">
          ...
          <div className="row">
            ...
            <div className="field">
              ...
              <input className="input" type="time" value={cutoff} onChange={(e) => setCutoff(e.target.value)} />
            </div>
          ...
          <div className="row">
            <div className="field">
              ...
              <input className="input" placeholder="DE" value={holidayCountry} onChange={(e) => setHolidayCountry(e.target.value)} />
            </div>
            <div className="field">
              <label className="muted">Holiday Region (optional)</label>
              <input className="input" placeholder="DE-BE" value={holidayRegion} onChange={(e) => setHolidayRegion(e.target.value)} />
            </div>
          </div>
          <div>
            ...
            {items.map((it, idx) => (
              <div key={idx} className="row" style={{ marginBottom: 6 }}>
                <input className="input" placeholder="Name" value={it.name} onChange={(e) => updateItem(idx, 'name', e.target.value)} />
                <input className="input" placeholder="Price (e.g. 1,50)" value={it.price} onChange={(e) => updateItem(idx, 'price', e.target.value)} style={{ width: 160 }} />
                <span className="muted">{currency}</span>
                <button onClick={() => removeItem(idx)} className="btn">Remove</button>
              </div>
            ))}
            <button onClick={addItem} className="btn" style={{ marginTop: 4 }}>Add Item</button>
          </div>

          <div className="row">
            <button onClick={() => create.mutate()} disabled={create.isPending || disabled} className="btn primary">
