...
export default function Calendar({ value, onChange, min, max, onClose, holidaysLabelByDate }: CalendarProps) {
  const todayStr = ymdFromDateUTC(new Date())
  const initial = value ? dateFromYmdUTC(value) : new Date()
  const [view, setView] = React.useState<Date>(new Date(Date.UTC(initial.getUTCFullYear(), initial.getUTCMonth(), 1)))

  const monthName = React.useMemo(() => {
    return new Intl.DateTimeFormat(undefined, { month: 'long', year: 'numeric', timeZone: 'UTC' }).format(view)
  }, [view])
  ...
  return (
    <div className="calendar-pop card" role="dialog" aria-label="Choose date">
      <div className="calendar-header">
        <button className="btn" onClick={() => setView(addMonthsUTC(view, -1))} aria-label="Previous month">◀</button>
        <div className="calendar-title">{monthName}</div>
        <button className="btn" onClick={() => setView(addMonthsUTC(view, 1))} aria-label="Next month">▶</button>
      </div>
      <div className="calendar-weekdays">
        {weekdays.map((d) => (
          <div key={d} className="muted mini" aria-hidden="true">{d}</div>
        ))}
      </div>
        ...
        {cells.map((c) => {
          ...
          return (
            <button
              ...
              onClick={() => { if (!disabled) { onChange(c.ymd); onClose?.() } }}
              disabled={disabled}
      ...
      <div className="calendar-footer">
        <button className="btn" onClick={() => { onChange(todayStr); onClose?.() }}>Today</button>
      </div>
    </div>
  )
}
