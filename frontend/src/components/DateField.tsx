...
export default function DateField({ value, onChange, min, max, disabled, className, style, holidaysLabelByDate }: Props) {
  ...
  return (
    <div ref={wrapRef} className={["date-field", disabled ? 'is-disabled' : '', className || ''].filter(Boolean).join(' ')} style={style}>
      <input
        ref={ref}
        type="date"
      ...
      />
      <button type="button" className="date-button" onClick={openPicker} aria-label="Open calendar" disabled={disabled}>
      ...
      {open && pos && createPortal(
        <div ref={portalRef} style={{ position: 'fixed', zIndex: 3000, top: pos.top, left: pos.left }}>
          <Calendar value={value} onChange={(d) => { onChange(d); setOpen(false) }} min={min} max={max} onClose={() => setOpen(false)} holidaysLabelByDate={holidaysLabelByDate} />
        </div>,
        document.body
      )}
    </div>
