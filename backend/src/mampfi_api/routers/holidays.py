...
def _cache_key(country: str, year: int, region: Optional[str]) -> str:
...
def _fetch_public_holidays(country: str, year: int) -> List[dict]:
    ...
    try:
        ...
        raise HTTPException(status_code=502, detail=f"holiday source error: {e.__class__.__name__}")


@router.get("/holidays")
def get_holidays(
    country: str = Query(..., min_length=2, max_length=8, description="ISO 3166-1 alpha-2 country code"),
    year: int = Query(..., ge=1900, le=2100),
    region: Optional[str] = Query(None, min_length=2, max_length=16, description="Optional regional/county code"),
) -> List[dict]:
    ...
    now = time.time()
    data_all: Optional[List[dict]] = None
